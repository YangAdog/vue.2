...
<template>
    ...
    <div class="calc flex flex-col gap-5">
        ...
        <div v-html="html"></div>
        <div>
            <label>數字A: <input v-model="numA" type="number"></label>
        </div>
        <div class="mb-[30px]">
            <label>數字B: <input v-model="numB" type="number"></label>
        </div>

        <div class="btns">
            <!-- 製作一個可以複用的，然後用:name把data的值帶入子元件內進行計算 -->
            <CalcButton :calc-type="calcType" :number-one="numA" :number-two="numB" @final-math="finalMath" />
        ...
        <!-- <button class="btn" type="button" @click="calcNumber('+')"> + </button>
            <button class="btn" type="button" @click="calcNumber('-')"> - </button>
            <button class="btn" type="button" @click="calcNumber('*')"> * </button>
            <button class="btn" type="button" @click="calcNumber('/')"> / </button> -->
        ...
        <div>
            ...
            <button type="button"
                class=" rounded-full bg-sky-600 text-white w-[50px] h-[50px] flex items-center justify-center">1</button>
        </div>
